...
ensure_go2_description_install_current() {
  log "Ensuring go2_description install is current for board_description."

  if ! command -v colcon >/dev/null 2>&1; then
    log "FAIL: colcon not found; cannot refresh install_container/go2_description."
    exit 1
  fi

  if [[ ! -f /opt/ros/humble/setup.bash ]]; then
    log "WARN: /opt/ros/humble/setup.bash missing on host; assuming install_container/go2_description is already current."
    return 0
  fi

  if ! bash -lc "source /opt/ros/humble/setup.bash && if [[ -f '${ROOT_DIR}/install_container/local_setup.bash' ]]; then source '${ROOT_DIR}/install_container/local_setup.bash'; fi && colcon build --build-base '${ROOT_DIR}/build_container_go2_description_sync' --install-base '${ROOT_DIR}/install_container' --packages-select go2_description" \
    >"${RUN_DIR}/00_go2_description_build.out.log" 2>"${RUN_DIR}/00_go2_description_build.err.log"; then
    log "FAIL: colcon build for go2_description failed."
    log "Inspect ${RUN_DIR}/00_go2_description_build.err.log"
    exit 1
  fi

  if ! python3 - "${ROOT_DIR}/install_container/go2_description/share/go2_description/urdf/go2.urdf" \
    >"${RUN_DIR}/00_go2_description_install_check.out.log" \
    2>"${RUN_DIR}/00_go2_description_install_check.err.log" <<'PY'
import sys
...
if "base_link" not in links:
    raise SystemExit("installed URDF is stale: base_link link missing")
...
for child_name in ("camera_link", "lidar_frame", "imu"):
    parent_name = parents.get(child_name)
    print(f"{child_name}<-{parent_name}")
    if parent_name != "base_link":
        raise SystemExit(
            f"installed URDF is stale: expected {child_name} parent base_link, got {parent_name}"
        )
PY
  then
    log "FAIL: install_container/go2_description does not match the expected base_link-rooted sensor URDF."
    log "Inspect ${RUN_DIR}/00_go2_description_install_check.err.log"
    exit 1
  fi

  log "PASS go2_description install check: install_container/go2_description is current."
}
...
load_extrinsics_defaults_if_needed() {
  local calibration_dir extrinsics_env calibration_snapshot manual_dir mapping_env
...
PY
      ...
      log "No camera calibration found; using camera_xyz/camera_rpy as currently set"
    fi
  else
  ...
  if [[ "$(normalize_triplet "${LIDAR_XYZ}")" == "0 0 0" && "$(normalize_triplet "${LIDAR_RPY}")" == "0 0 0" ]]; then
    ...
    if [[ -n "${extrinsics_env}" && -f "${extrinsics_env}" ]]; then
      calibration_snapshot="${RUN_DIR}/00_lidar_mount_calibration.env"
      python3 - "${extrinsics_env}" "${calibration_snapshot}" <<'PY'
import shlex
import sys
from pathlib import Path
...
PY
      ...
      log "No lidar calibration found; using lidar_xyz/lidar_rpy as currently set"
    fi
  else
...
wait_realsense_ready() {
  ...
  local expected_topics="${ARTIFACT_DIR}/04_realsense_topics_expected.log"
  ...
  while true; do
    ...
    if [[ "${list_rc}" -eq 0 ]] \
      && grep -Fxq "${RS_RGB_TOPIC}" "${ARTIFACT_DIR}/04_realsense_topics_full.log" \
      && grep -Fxq "${RS_DEPTH_TOPIC}" "${ARTIFACT_DIR}/04_realsense_topics_full.log" \
      && grep -Fxq "${RS_CAMERA_INFO_TOPIC}" "${ARTIFACT_DIR}/04_realsense_topics_full.log"; then
      set +e
      timeout 6s ros2 topic echo --once "${RS_CAMERA_INFO_TOPIC}" \
...
BOARD_PID="$(start_bg \
  "ros2 launch launch/board_description.launch.py lidar_frame:=${LIDAR_FRAME} lidar_xyz:='${LIDAR_XYZ}' lidar_rpy:='${LIDAR_RPY}' camera_frame:=${CAMERA_FRAME} camera_xyz:='${CAMERA_XYZ}' camera_rpy:='${CAMERA_RPY}'" \
...
if wait_realsense_ready 60; then
  ...
  grep -E "realsense|image_raw|camera_info" "${ARTIFACT_DIR}/04_realsense_topics_full.log" >"${ARTIFACT_DIR}/04_realsense_topics_filtered.log" || true
  {
    ...
    cat "${ARTIFACT_DIR}/04_realsense_topics_expected.log" 2>/dev/null || true
    echo
    echo "Observed camera topics:"
    grep '^/camera/' "${ARTIFACT_DIR}/04_realsense_topics_full.log" 2>/dev/null || true
  } >"${ARTIFACT_DIR}/04_realsense_topics_debug.log"
  ...
  cat "${ARTIFACT_DIR}/04_realsense_topics_debug.log" | tee -a "${ARTIFACT_DIR}/summary.log" >/dev/null || true
  exit 1
fi
...
LOCALIZATION_PID="$(start_bg \
  "ros2 launch launch/rtabmap_localization.launch.py database_path:='${DB_PATH}' rgb_topic:=${RS_RGB_TOPIC} depth_topic:=${RS_DEPTH_TOPIC} camera_info_topic:=${RS_CAMERA_INFO_TOPIC} scan_topic:=${SCAN_FRESH_TOPIC} base_frame:=base_link odom_frame:=odom map_frame:=map" \
